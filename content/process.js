...
(function() {
  ...
  const LOGIN_API = 'https://staging-seekr-adaptive-api.projectbasta.com/auth/login';
  ...
  function extractFromLinkedIn() {
      ...
      try {
        ...
        for (const compElement of compElements) {
          ...
          if (isValidCompensationText(compText)) {
            const m = compText.match(/\$[\d,]+[KMB]?\s*\/?\s*(yr|year|hr|hour)?\s*-?\s*\$?[\d,]*[KMB]?\s*\/?\s*(yr|year|hr|hour)?/i) ||
                     compText.match(/\$[\d,]+[KMB]?/);
            jobData.compensation = m ? m[0].trim() : compText.slice(0, 150).trim();
            break;
          }
        }
    ...
    if (!jobData.compensation) {
      const primaryDesc = document.querySelector('.jobs-details-top-card__primary-description, .job-details-jobs-unified-top-card__primary-description, .jobs-unified-top-card__primary-description');
      if (primaryDesc && isElementVisible(primaryDesc)) {
        ...
        if (isValidCompensationText(descText) && descText.length < 150) {
          const m = descText.match(/\$[\d,]+[KMB]?\s*\/?\s*(yr|year|hr|hour)?\s*-?\s*\$?[\d,]*[KMB]?/i);
      ...
      for (const sel of insightSelectors) {
        ...
        for (const el of nodes) {
          ...
          if (isValidCompensationText(fullText) && fullText.length < 250) {
            const m = fullText.match(/\$[\d,]+[KMB]?\s*\/?\s*(yr|year|hr|hour)?\s*-?\s*\$?[\d,]*[KMB]?\s*\/?\s*(yr|year|hr|hour)?/i);
            jobData.compensation = m ? m[0].trim() : fullText.slice(0, 120).trim();
            break;
          }
        }
        if (jobData.compensation) break;
    ...
    if (!jobData.compensation) {
      const mainDesc = document.querySelector('.jobs-description__text, .jobs-description-content__text, .jobs-box__html-content, [data-testid="job-description"]');
      if (mainDesc && isElementVisible(mainDesc)) {
        const text = mainDesc.textContent.trim().replace(/\s+/g, ' ');
        const m = text.match(/\$[\d,]+[KMB]?\s*\/?\s*(yr|year|hr|hour)?\s*-?\s*\$?[\d,]*[KMB]?\s*\/?\s*(yr|year|hr|hour)?/i);
    ...
    if ((!jobData.jobTitle || !jobData.companyName) && document.title) {
      ...
      } else if (parts.length >= 2 && parts[parts.length - 1].toLowerCase() === 'linkedin') {
        if (!jobData.jobTitle) jobData.jobTitle = parts[0];
      }
    }
  ...
  function removeSidebar() {
    ...
    document.body.classList.remove('basta-sidebar-active', 'basta-sidebar-hidden-body');
  }
  ...
  function openMenuDropdown(sidebar) {
    ...
    if (!dropdown) return;
    chrome.storage.local.get(['authToken', 'tokenExpiration', 'userFirstName', 'userEmail'], (result) => {
      const isLoggedIn = result.authToken && result.tokenExpiration && Date.now() < result.tokenExpiration;
      dropdown.innerHTML = renderMenuContent(isLoggedIn, result.userFirstName, result.userEmail);
      dropdown.classList.add('basta-sidebar-menu-open');

      if (isLoggedIn) {
        const logoutBtn = dropdown.querySelector('#basta-sidebar-logout');
        if (logoutBtn) logoutBtn.addEventListener('click', handleSidebarLogout);
      } else {
  ...
  function closeMenuDropdown(sidebar) {
    const dropdown = sidebar && sidebar.querySelector('.basta-sidebar-menu-dropdown');
  ...
  function handleSidebarLogin(e) {
    ...
    fetch(LOGIN_API, {
      ...
      .then(res => res.ok ? res.json() : res.json().then(j => Promise.reject(new Error(j.message || 'Login failed'))))
      .then((responseData) => {
  ...
  function handleSidebarLogout() {
    chrome.storage.local.remove(['authToken', 'tokenExpiration', 'username', 'userFirstName', 'userEmail', 'userId', 'responseId']);
  ...
  function ensureExpandTabExists() {
    ...
    expandTab.style.display = sidebar.classList.contains('basta-sidebar-hidden') ? 'block' : 'none';
  ...
  function createSidebarInner() {
    ...
    if (existingSidebar) {
      ...
      if (menuBtn && !menuBtn.hasAttribute('data-listener-attached')) {
        ...
        menuBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          const dropdown = existingSidebar.querySelector('.basta-sidebar-menu-dropdown');
          const isOpen = dropdown && dropdown.classList.toggle('basta-sidebar-menu-open');
    ...
    document.addEventListener('click', (e) => {
      if (!sidebar.contains(e.target) && e.target !== expandTab) closeMenuDropdown(sidebar);
    });
    ...
    if (hideBtn) hideBtn.addEventListener('click', () => toggleSidebar(false, true));
  ...
  function loadSidebarContent(forceApiCall = false) {
    chrome.storage.local.get(['authToken', 'tokenExpiration'], (authResult) => {
      const isLoggedIn = authResult.authToken && authResult.tokenExpiration && Date.now() < authResult.tokenExpiration;
      const sidebar = document.getElementById(SIDEBAR_ID);
      if (!sidebar) return;
